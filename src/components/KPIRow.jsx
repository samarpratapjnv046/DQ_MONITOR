  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '8px' }}>
      <KPICard delay={1} color="var(--green)" icon="⬡"
        label="Weighted Health Score" sublabel="(Schema + Table + DQ combined)"
        value={`${m.healthScore}%`} sub="60% DQ · 30% Schema · 10% Table" />
      ...
      <KPICard delay={4} color="var(--red)" icon="⚑"
        label="Failed Validation Rules" sublabel="(Schema + DQ failures combined)"
      ...
      <KPICard delay={6} color="var(--purple)" icon="△"
        label="Tables Needing Fixes" sublabel="(schema mismatch, DQ fail, or skips)"
        value={`${m.tablesNeedingFixes}%`} sub={`${m.tablesWithFailures} of ${m.totalTables} tables flagged`} />
    </div>
  );
}
